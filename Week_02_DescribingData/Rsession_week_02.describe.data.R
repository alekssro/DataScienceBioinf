...
d = read_delim(file="./DataScienceBioinf/Week_02.Describing.data/1000g.allele.frequencies.tsv.gz",delim=c("\t"))

#### Counting SNPs ####

# Q: What is the distribution of the variable "frequency"?
ggplot(data = d, mapping = aes(x = frequency)) +
...
d %>% 
    group_by(population) %>%
    ggplot(d_bypopulation, mapping = aes(x = frequency, color = population)) +
        geom_freqpoly(bins = 100)

ggplot(d, mapping = aes(x = frequency, color = population)) +
    geom_freqpoly() +
...
#Something to do with polymorphisms
#It means that the probability of finding a certain SNP is higher than the probability of 
#not finding the SNP in that position
...
sum(fixed_SNPs[1, 2], fixed_SNPs[2:3, 3])
...
ggplot(d) +
    geom_point(mapping = aes(x = position, y = maf, color = population))
