...
```{r}
setwd(dir = "/mnt/22FA4665FA463577/aleks/Documents/Nextcloud/Universidad/Bioinformatics/DataScienceBioinf/Week_03_SamplingDistributions/") #ADapt to your path here
...
Species     | Identifies the spiece where the gene is from
labs        | Identifier of Specie-chromosome type
chrMark     | Chromosome Type (X / A)
chr         | Chromosome number
dN          | Non-synonymous substitutions
dS          | Synonymous substitutions
dNdS        | Rate of non-synonymous over synonymous substitutions (dN/dS)
RPKM        | Reads Per Kilobase per Million base pairs (normalized read counts/expression measurement)
Tau         | Expression specificity*
GC3         | GC content computed at third codon positions

------

*$$\tau = \frac{\sum_{i}^{N(t)} 1 - (RPKM_i/RPKM_{max})}{N(t) - 1}$$
...
mammals %>%
    ...
    ggplot(mapping = aes(x = RPKM)) +
    geom_histogram(binwidth = 100, color = "black", fill = "brown")
```
...
```{r}
mammals %>%
    ggplot(mapping = aes(x = dN, y = dS, color = chrMark)) +
    geom_point()
```
...
mammals %>%
    ...
    summarise(median_RPKM = median(RPKM), dNdS = log(sum(dN) / sum(dS))) %>%
    ggplot() +
    geom_point(mapping = aes(x = median_RPKM, y = dNdS, color = Species, shape = chrMark)) +
    geom_smooth(aes(x = median_RPKM, y = dNdS, color = Species), method = "lm", se = F) +
    labs(x = "dN/dS", y = "median RPKM")
