##Thi sis a practice R file from TB ... can be safely deleted after Nov 16
...
## Let's examine what happens without the US 

#!end
  
pdWoUS = pd %>% filter(Country !="United States")
dim(pdWoUS)

fit = lm(homicides ~ guns_pr_capita, data = pdWoUS) 
summary(fit)

ggplot(data = pdWoUS, mapping = aes(x = guns_pr_capita, y = homicides, label=Country)) +
  geom_smooth(method="lm", se=TRUE, lty="dashed", color="#808080", fill="#d0d0d0")+
  geom_point(size=3)+
  geom_text_repel()+
  ggtitle("Gun ownership vs. homicide rate in industrialized countries")
...
plot(fit)
pdWoUS[12,] ##Who's the biggest outlier
